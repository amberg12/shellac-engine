...
template <typename T, typename Enable = void> struct safe_larger_integral;
...
struct safe_larger_integral<T, std::enable_if_t<std::is_enum_v<T>>>
{
  ...
  using type = std::
    conditional_t<std::is_signed_v<underlying>, std::int64_t, std::uint64_t>;
};
...
constexpr auto to_safe_larger_integral(T e) -> safe_larger_integral_t<T>
{
  if constexpr (std::is_enum_v<T>) {
    ...
    return static_cast<typename TSliInfo::type>(
      static_cast<typename TSliInfo::underlying>(e)
    );
  } else {
